...
//make validation
  ...
  <div className="container">
    ...
    <form action="" onSubmit={submitUser} >
        ...
        {specificError("name")?<div className='alert alert-danger'>{specificError("name")}</div>:""}
        <label htmlFor="email">email</label>
        <input onChange={getUser}  className='form-control mb-3' type="email" id="email" placeholder='you@example.com' />
        {specificError("email")?<div className='alert alert-danger'>{specificError("email")}</div>:""}
        ...
        {specificError("password")?<div className='alert alert-danger'>Password must be between 3 to 9 characters long and can only contain letters and numbers.</div>:""}

 
        <label htmlFor="rePassword">confirm password</label>
        <input  onChange={getUser} className='form-control mb-3' type="password" id="rePassword" placeholder='confirm password' />
        {specificError("rePassword")?<div className='alert alert-danger'>'Passwords do not match.</div>:""}

        <label htmlFor="phone">phone</label>
        <input  onChange={getUser} className='form-control mb-3' type="tel" id="phone" placeholder='enter phone' />
        {specificError("phone")?<div className='alert alert-danger'>{specificError("phone")}</div>:""}

        <button className='btn btn-outline-info'>{reloadButton === false? "Register":<i className='fa-solid fa-spinner fa-spin '></i>}</button>
