...
function MovieDetails() {
  ...
  return (
    <>
    {details? <div className="container w-100">
            ...
            <div className="moviedetails d-flex flex-column gap-2">
              ...
              <div className="hashtag d-flex gap-3">
                {details.genres.map((hash,idx)=>
                    <span key={idx
                    } className="bg-info px-4 py-1 rounded">{hash.name}</span>

                )}

              </div>
              <h5> vote :  {details.vote_average.toFixed(1)} <i style={{color:"yellow"}} class="fa-solid fa-star"></i> </h5>
